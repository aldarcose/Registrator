...
namespace DBFCheck
{
    class WorkingDBF
    {
        public DataTable getDataTableFromDBF(string path, string file)
        {
            string connectionString = @"Provider=VFPOLEDB.1;Data Source=" + path + file + "";
        ...
        public void Append(DataTable DT, string FileName)
        {
            DataTable table = WorkingDBF.Load(FileName);
            table.BeginLoadData();
            foreach (DataRow r in DT.Rows)
                table.Rows.Add(r.ItemArray);
            table.EndLoadData();
            table.TableName = Path.GetFileNameWithoutExtension(FileName);
            WorkingDBF.Save(table, Path.GetDirectoryName(FileName));
        }
    }
}
